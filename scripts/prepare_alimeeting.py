...
def overlap_regions_from_turns(turns: list[Turn]) -> list[Region]:
    """Return spans where >= 2 speakers are simultaneously active (sweep line)."""
    ...
    for timestamp, delta in events:
        if active >= 2 and timestamp > previous and region_start is not None:
            regions.append((region_start, timestamp))
            region_start = None
        active += delta
        if active >= 2 and region_start is None:
