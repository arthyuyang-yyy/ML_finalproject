...
| 语音分离 | `src/speech_separation.py`、`src/nmf_separation.py` | 可替换适配器：零依赖自写 NMF 基线（`nmf`）+ 可选 SpeechBrain SepFormer（`sepformer`），默认关闭并保留安全回退 |
| 高重叠路径 | `src/high_overlap.py` | 优先将可选分离声源分别转写为候选；否则使用多参数候选，主记录保持 mixed/空转写 |
